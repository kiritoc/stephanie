	...
	function get_post_meta_box($post) {
		...
		if ($post->post_type != 'plugin_filter') {
			...
			foreach($availableRoles as $roleID=>$roleName) {
				...
				if ($fuzzyPermalink != '' && get_option('PO_disable_mobile_plugins') == '1' && get_option("PO_fuzzy_url_matching") == "1" && sizeof($pluginLists[$roleID]['disabled_mobile_plugin_list']) == 0 && sizeof($pluginLists[$roleID]['enabled_mobile_plugin_list']) == 0 && sizeof($pluginLists[$roleID]['disabled_mobile_group_list']) == 0 && sizeof($pluginLists[$roleID]['enabled_mobile_group_list']) == 0) {
