...
class PluginOrganizer {
	...
	function check_version() {
		...
		if (get_option("PO_version_num") != "9.0.4" && !in_array($pagenow, array("plugins.php", "update-core.php", "update.php"))) {
			$this->activate();
		}
	}
	...
	function activate() {
		...
		$newColumns = array(
			...
			'user_role' => array(0, "varchar(255) NOT NULL default '_'")
		);
		...
		if (get_option("PO_version_num") != "9.0.4") {
			update_option("PO_version_num", "9.0.4");
		}
	...
	function find_duplicate_permalinks($postID, $permalink, $secure) {
		global $wpdb;
		$returnDup = array();
		$dupPostQuery = "SELECT post_id FROM ".$wpdb->prefix."po_plugins WHERE permalink = %s and post_id != %d AND secure = %d status != 'trash' GROUP BY post_id";
	...
	function save_post_meta_box($post_id) {
		...
		$deletePermalinksRes = $wpdb->get_results($wpdb->prepare("SELECT permalink, secure FROM ".$wpdb->prefix."po_plugins WHERE post_id=%d GROUP BY permalink, secure", $post_id), ARRAY_A);
		...
		if (get_post_type($post_id) != 'plugin_filter') {
			foreach($availableRoles as $roleID=>$roleName) {
				$pluginListID = $wpdb->get_var($wpdb->prepare("SELECT pl_id FROM ".$wpdb->prefix."po_plugins WHERE post_id=%d AND user_role=%s", $post_id, $roleID));
				if (is_numeric($pluginListID)) {
					$permalinks[] = array($pluginListID, get_permalink($post_id), $roleID);
				} else {
					$permalinks[] = array('tmp', get_permalink($post_id), $roleID);
				}
			}
		} else {
			if (isset($_POST['PO_pl_id'])) {
				foreach($_POST['PO_pl_id'] as $plID) {
					...
					foreach($availableRoles as $roleID=>$roleName) {
						if (!is_numeric($plID)) {
							$permalinks[] = array($plID, $this->fix_trailng_slash($_POST['PO_permalink_filter_'.$plID]), $roleID);
						} else {
							$pluginListID = $wpdb->get_var($wpdb->prepare("SELECT pl_id FROM ".$wpdb->prefix."po_plugins WHERE post_id=%d AND user_role=%s AND permalink=%s AND secure=%d", $post_id, $roleID, $oldPermalink['permalink'], $oldPermalink['secure']));
							if (!is_numeric($pluginListID)) {
								$permalinks[] = array('tmp_tmp', $this->fix_trailng_slash($_POST['PO_permalink_filter_'.$plID]), $roleID);
							} else {
								$permalinks[] = array($pluginListID, $this->fix_trailng_slash($_POST['PO_permalink_filter_'.$plID]), $roleID);
							}
						}
					}
				}
		...
		foreach($permalinks as $permalink) {
			...
			if ($permalink[1] != '' && get_option("PO_fuzzy_url_matching") == "1" && get_post_type($post_id) != 'plugin_filter') {
				$fuzzyPluginList = $this->find_parent_plugins($post_id, $permalink[1], 0, $secure, $permalink[2]);
				...
				$fuzzyMobilePluginList = $this->find_parent_plugins($post_id, $permalink[1], 1, $secure, $permalink[2]);
		...
		foreach($deletePermalinks as $deletePermalink) {
			...
			$wpdb->query($wpdb->prepare($deletePluginQuery, $permalink, $secure, $post_id));
	...
	function get_available_roles() {
		if (get_option("PO_disable_by_role") == '1') {
	...
	function get_submitted_plugin_lists($role) {
		...
		$checkPluginList = (isset($_POST['PO_disabled_std_plugin_list'][$role])) ? $_POST['PO_disabled_std_plugin_list'][$role] : '';
		...
		if (get_option('PO_disable_mobile_plugins') == 1) {
			...
			$checkPluginList = (isset($_POST['PO_disabled_mobile_plugin_list'][$role])) ? $_POST['PO_disabled_mobile_plugin_list'][$role] : '';
		...
		$checkPluginList = (isset($_POST['PO_disabled_std_group_list'][$role])) ? $_POST['PO_disabled_std_group_list'][$role] : '';
		...
		if (get_option('PO_disable_mobile_plugins') == 1) {
			...
			$checkPluginList = (isset($_POST['PO_disabled_mobile_group_list'][$role])) ? $_POST['PO_disabled_mobile_group_list'][$role] : '';
