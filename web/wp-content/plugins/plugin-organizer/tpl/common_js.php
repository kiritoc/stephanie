	...
	jQuery(function() {
		...
		jQuery('.outerPluginWrap.disabledPlugins').droppable({
			...
			drop: function(event, ui) {
				...
				var roleName = jQuery(this).find('.user_role_name').val();
				if (newElement.hasClass('pluginWrap')) {
					var pluginID = newElement.find('.PO-plugin-id').val();
					if (jQuery(this).find(':hidden[value="'+pluginID+'"]').length == 0) {
						var targetType = '';
						if (jQuery(this).hasClass('PO-disabled-mobile-plugin-wrap')) {
							targetType = 'mobile';
						...
						} else if (jQuery(this).hasClass('PO-disabled-std-plugin-wrap')) {
							targetType = 'std';
							newElement.find('.PO-disabled-item-id').remove();
						...
						var pluginWrapper = jQuery(this).find(':hidden[value="'+pluginID+'"]').closest('.pluginWrap');
			...
			drop: function(event, ui) {
				if (ui.draggable.closest('#PO-all-plugin-wrap').length < 1) {
					if (ui.draggable.find('.PO-disabled-item-id').hasClass('PO-disabled-mobile-plugin-list')) {
						PO_deactivate_indicator('plugin', 'mobile', ui.draggable.find('.PO-plugin-id').val(), ui.draggable.closest('.outerPluginWrap.disabledPlugins').find('.user_role_name').val());
					} else if (ui.draggable.find('.PO-disabled-item-id').hasClass('PO-disabled-std-plugin-list')) {
						PO_deactivate_indicator('plugin', 'std', ui.draggable.find('.PO-plugin-id').val(), ui.draggable.closest('.outerPluginWrap.disabledPlugins').find('.user_role_name').val());
					} else if (ui.draggable.find('.PO-disabled-item-id').hasClass('PO-disabled-mobile-group-list')) {
						PO_deactivate_indicator('group', 'mobile', ui.draggable.find('.PO-group-id').val(), ui.draggable.closest('.outerPluginWrap.disabledPlugins').find('.user_role_name').val());
					} else if (ui.draggable.find('.PO-disabled-item-id').hasClass('PO-disabled-std-group-list')) {
						PO_deactivate_indicator('group', 'std', ui.draggable.find('.PO-group-id').val(), ui.draggable.closest('.outerPluginWrap.disabledPlugins').find('.user_role_name').val());
					}
